...
function App() {
  ...
  const addPlanToGrocery = () => {
    const allRecipeNames = Object.values(mealPlan).flat();
    ...
    <div className={`app ${darkMode ? "dark" : ""}`}>
      {showWelcome && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ background: "var(--surface)", borderRadius: "12px", padding: "32px 28px", maxWidth: "340px", textAlign: "center" }}>
            ...
            <button onClick={() => { localStorage.setItem("instacook_welcomed", "true"); setShowWelcome(false); setMobileTab("pantry"); }} style={{ background: "var(--accent)", color: "white", border: "none", borderRadius: "8px", padding: "12px 24px", fontSize: "14px", fontWeight: "700", cursor: "pointer", width: "100%", marginBottom: "10px", fontFamily: "Outfit, sans-serif" }}>Set Up My Pantry</button>
            <button onClick={() => { localStorage.setItem("instacook_welcomed", "true"); setShowWelcome(false); }} style={{ background: "transparent", color: "var(--muted)", border: "none", fontSize: "13px", cursor: "pointer", fontFamily: "Outfit, sans-serif" }}>Skip for now</button>
          </div>
        </div>
      )}
      <header className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="btn-planner-toggle" onClick={() => setShowPlanner(true)}>
          ...
          <div className={`detail-section ${mobileTab === "grocery" ? "mobile-hidden" : ""}`}>
            ...
            {!selectedRecipe ? (
              ...
              <>
                ...
                <button className="btn-add-plan" onClick={() => { setPlannerAddRecipe(selectedRecipe); setShowPlanner(true); }}>📅 Add to Meal Plan</button>
